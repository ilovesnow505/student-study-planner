...
from database import (
    create_table, add_assignment, get_assignments, 
    update_status, delete_assignment
)
...
menu = st.sidebar.radio("Navigation", ["Dashboard", "Add Assignment", "View Assignments", "Update Assignment"])
...
if menu == "Dashboard":
    ...
    if assignments:
        df = pd.DataFrame(assignments, columns=["ID", "Subject", "Assignment", "Due Date", "Priority", "Status"])
        total = len(df)
...
# ADD ASSIGNMENT
    ...
    if st.button("Add Assignment"):
        if subject and assignment:
            add_assignment(subject, assignment, str(due_date), priority, status)
...
# UPDATE ASSIGNMENT
    ...
    if assignments:
        assignment_dict = {f"{row[0]} - {row[2]}": row[0] for row in assignments}
        selected = st.selectbox("Select Assignment", list(assignment_dict.keys()))
        new_status = st.selectbox("New Status", ["Pending", "In Progress", "Completed"])
        
        if st.button("Update Status"):
            assignment_id = assignment_dict[selected]
            update_status(assignment_id, new_status)
